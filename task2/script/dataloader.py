...
def test(net, data, labels, batch_size = 100, tops = [1,5],
    ...
    with torch.no_grad():
        for x, label in dataloader(data, labels, batch_size = batch_size, 
            ...
            # labelst = torch.tensor(label, dtype = torch.float32, device = cuda) #label的shape为(100,)
            # print(labelst.shape)
            ...
            # y0 = net(x.to(cuda))
            # y1 = torch.nn.Softmax(dim=-1)(y0)
            # losst = []
  

            # loss = torch.nn.functional.binary_cross_entropy(y1, labelst)
