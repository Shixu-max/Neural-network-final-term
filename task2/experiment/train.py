...
for epoch in range(len(accs) + 1, epochs + len(accs) + 1):
    for x , labels in dataloader(train_x, train_y, CONFIG.train_size,
        ...
        y = net(x.to(cuda)) 
        labels = torch.tensor(labels, dtype = torch.float32, device = cuda)# label.shape为[256,100]
    ...
    with torch.no_grad():
        for x , labels in dataloader(test_x, test_y, CONFIG.test_size,
                                 cut = CONFIG.cut, mix = CONFIG.mix, onehot = True,
                                 jitter = CONFIG.jitter):
            y = net(x.to(cuda))
            labels = torch.tensor(labels, dtype = torch.float32, device = cuda)# label.shape为[256,100]
    ...
    # losst.append(test(net, test_x, test_y, CONFIG.test_size, cuda = cuda))
    ...
    #for j in range(len(losst) - n // CONFIG.test_size, len(losst)):
    for j in range( len(losst)):
        writer.add_scalar(tag="test/loss", global_step = j, scalar_value = losst[j])   
